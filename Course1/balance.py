...
INPUT_FILE = os.path.join(BASE_DIR, "betlist.txt")
SUCCESS_FILE = os.path.join(BASE_DIR, "successbalance16.txt")
ERROR_FILE = os.path.join(BASE_DIR, "errorlistbalance5.txt")
TOOMANY_FILE = os.path.join(BASE_DIR, "toomanybalance5.txt")
PENDING_FILE = os.path.join(BASE_DIR, "pendingbalance5.txt")
...
def check_balance(session, dt):
    ...
    try:
        ...
        if cc:
            bal_url = "https://bonus.imarabet.co.ke:443/bonus/balance"
            bal_headers = headers.copy()
            bal_headers.update({"Api-Key": cc, "Accept": "*/*"})

            resp1 = session.get(bal_url, headers=bal_headers, timeout=TIMEOUT)
            mm = resp1.json()
